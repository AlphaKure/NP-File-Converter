    ...
    if os.path.isdir(Path):
        ...
        for Dir in os.listdir(Path):
            ...
            if HaveUltima:
                ...
                Data.find('name').str.string = Data.find(
                    'name').str.string+'(ULTIMA)'
            ...
            # 以下為共同修改
            RightInfo = BeautifulSoup(
                '<rightsInfoName><id>0</id><str>なし</str><data /></rightsInfoName>', 'xml')

            # PreviewTime
            if tool.ReadSetting('PreviewTime', 'GetPreviewTime').lower() == 'false':
                PreviewStartTag = BeautifulSoup(
                    '<previewStartTime>50000</previewStartTime>', 'xml')
                PreviewEndTag = BeautifulSoup(
                    '<previewEndTime>75000</previewEndTime>', 'xml')
            else:
                StartTime, EndTime = tool.FindPreviewTime(
                    Data.find('jaketFile').path.string[14:18])
                PreviewStartTag = BeautifulSoup(
                    '<previewStartTime>'+StartTime+'</previewStartTime>', 'xml')
                PreviewEndTag = BeautifulSoup(
                    '<previewEndTime>'+EndTime+'</previewEndTime>', 'xml')
            ...
            if HaveUltima:
                ...
                for File in os.listdir(NowDir):
                    if File.endswith('.c2s'):
                        os.rename(NowDir+File, NowDir +
                                  f'{NewNumber}_0{Count}.c2s')
                        Count += 1
                    elif File.endswith('.dds'):
                        os.rename(NowDir+File, NowDir +
                                  f'CHU_UI_Jacket_{NewNumber}.dds')
